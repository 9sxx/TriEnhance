...
class TriEnhanceClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X_train, y_train, X_unlabeled):
        ...
        # 合并正确分类的样本,错误分类的样本与最佳模式下平衡后的数据
        X_combined = pd.concat([X_train_split_balanced, correctly_classified_samples_best, misclassified_samples_best])
        y_combined = pd.concat([y_train_split_balanced, correctly_classified_labels_best, misclassified_labels_best])
    ...
    def _enhance_training_set(self, X_train, y_train, X_unlabeled, target_sample_percent=0.3):
        """
        通过未标记数据增强训练集。
